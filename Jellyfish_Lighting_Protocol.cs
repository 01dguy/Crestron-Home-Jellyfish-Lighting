...
{
	public class Jellyfish_Lighting_Protocol : ABaseDriverProtocol
	{
		...
		public void Start()
		{
			...
			{
				LastStatus = string.IsNullOrEmpty(TransportLayer.LastTransportError)
					? "Disconnected: Set ControllerHost user attribute."
					: "Disconnected: " + TransportLayer.LastTransportError;
		...
		public void SetPowerState(int state)
		{
			...
			if ((LastZoneNames == null || LastZoneNames.Length == 0) && KnownZones.Length > 0)
			{
			...
			{
				LastStatus = "Power command requires a selected pattern and one or more zones.";
				UI_Update?.Invoke();
				return;
			}
		...
		public void HandleInboundWebSocketJson(string json)
		{
			...
			var commandType = ExtractString(json, "cmd");
			if (!string.Equals(commandType, "fromCtlr", StringComparison.OrdinalIgnoreCase))
			{
				return;
			}
